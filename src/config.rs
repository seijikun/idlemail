...
impl ConfigContainer {
	pub fn from_file<P: AsRef<Path>>(path: P) -> Result<ConfigContainer, String> {
		...
		let config: ConfigContainer = serde_json::from_reader(reader).map_err(|e| format!("Failed to parse config file: {}", e))?;
		config.validate()?;
		Ok(config)
	}
...
pub enum SourceConfig {
	...
	#[serde(rename = "imap_poll")] ImapPoll (ImapPollSourceConfig),
	#[serde(rename = "imap_idle")] ImapIdle (ImapIdleSourceConfig)
}
