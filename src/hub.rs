...
use crate::{
	destinations::{testdst::TestDestination, smtp::SmtpDestination},
	sources::{
		imap_poll::ImapPollSource,
		imap_idle::ImapIdleSource,
		testsrc::TestSource
	}, config::RetryAgentConfig, retryagents::{filesystem::FilesystemRetryAgent, memory::MemoryRetryAgent}
};
use log::{info, warn};
use async_std::{task, sync as async_mpsc, future::timeout as await_timeout};
use std::{
	sync::mpsc,
	collections::{hash_map::DefaultHasher, HashMap},
	time::Duration, hash::{Hasher, Hash}
};
use mpsc::RecvError;
...
impl HubChannel {
	pub fn new() -> Self {
		...
		Self {
			...
			sources: HashMap::new(),
			retryagent_sender, retryagent_recv: Some(retryagent_recv)
		}
	}
	...
	pub fn queue_mail_for_sending(&self, dstname: &str, mail: Mail) -> Result<(), ()> {
		...
		dst_comm.send(DestinationMessage::Mail { mail })
			.map_err(|_| ())
	}

	pub fn queue_mail_for_retry(&self, dstname: String, mail: Mail) {
		if let Err(_) = self.retryagent_sender.send(RetryAgentMessage::QueueMail { dstname, mail }) {
	...
	pub fn shutdown_retryagent(&self) {
		...
		self.retryagent_sender.send(RetryAgentMessage::Shutdown).unwrap();
	}
...
impl HubDestinationChannel {
	...
	pub fn notify_failed_send(&self, mail: Mail) {
		self.sender.send(HubMessage::SendingMailFailed { dstname: self.name.clone(), mail }).unwrap();
	}
}
...
impl HubSourceChannel {
	...
	pub fn notify_new_mail(&self, mail: Mail) {
		self.sender.send(HubMessage::NewMail { srcname: self.name.clone(), mail }).unwrap();
	}
}
...
impl HubRetryAgentChannel {
	...
	pub fn notify_retry_mail(&self, dstname: String, mail: Mail) {
		self.sender.send(HubMessage::RetryMail { dstname, mail }).unwrap();
	}
}
	...
	pub fn from_config(config: &ConfigContainer) -> Self {
		...
		for (dstname, dstcfg) in &config.destinations {
			let destination_agent: Box<dyn MailDestination> = match dstcfg {
				DestinationConfig::Test(config) => Box::new(TestDestination::new(dstname.clone(), config)),
		...
		for (srcname, srccfg) in &config.sources {
			let source_agent: Box<dyn MailSource> = match srccfg {
				...
				SourceConfig::ImapPoll(config) => Box::new(ImapPollSource::new(srcname.clone(), config)),
				SourceConfig::ImapIdle(config) => Box::new(ImapIdleSource::new(srcname.clone(), config))
			};
		...
		Self {
			destination_agents, source_agents, retryagent,
				...
				HubMessage::NewMail { srcname, mail } => {
					...
					if let Some(dstlist) = self.mappings.get(&srcname) {
						for dstname in dstlist {
							...
							self.hubchannel.queue_mail_for_sending(dstname, mail.clone())
