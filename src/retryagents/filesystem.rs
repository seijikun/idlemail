...
use crate::{
	...
	hub::{MailAgent, MailRetryAgent, Mail, RetryAgentMessage}
};
use log::{debug, info, warn, error};
use std::{collections::VecDeque, thread, time::{Duration, SystemTime}, fs};
...
impl MailRetryAgent for FilesystemRetryAgent {
    fn start(&mut self, channel: crate::hub::HubRetryAgentChannel) {
		...
		info!(target: &log_target, "Loading messages from folder: {}", config.path);
		let mut restored_mails = match self.load_from_fs() {
			Ok(restored_mails) => restored_mails,
			Err(e) => {
				error!(target: &log_target, "Failed to load retry-mails from filesystem:\n{}", e);
				Vec::new()
			}
		};
			...
			loop {
				if let Some(msg) = channel.next_timeout(Duration::from_secs(1)) {
					...
					match msg {
						...
						RetryAgentMessage::QueueMail { dstname, mail } => {
							...
							info!(target: &log_target, "Queueing mail {} for retransmission in {}s", mail.hash, config.delay);
							...
							for i in 0..10 { // try 10 append-indices against hash-collision
								...
								if let Ok(retry_file) = fs::File::create(&file_name) {
									...
									match serde_json::to_writer(retry_file, &QueuedRetryMailModel::from(&retry_mail)) {
										Ok(_) => {
											debug!(target: &log_target, "Stored retry-mail in: {}", retry_mail.file_path);
											queue.push_back(retry_mail);
											break;
										},
										Err(e) => {
											error!(target: &log_target, "Failed to create retry-mail file: {}\n{}", retry_mail.file_path, e);
											continue;
										}
									}
								}
				...
				while queue.len() > 0 {
					if queue.get(0).unwrap().due_time < now {
						...
						info!(target: &log_target, "Mail {} due for retransmission. Queueing.", mail.mail.hash);
						channel.notify_retry_mail(mail.dstname, mail.mail);
						if let Err(e) = fs::remove_file(&mail.file_path) {
							warn!(target: &log_target, "Failed to delete retry-mail file: {}\n{}", mail.file_path, e);
						} else {
							debug!(target: &log_target, "Deleted retry-mail file: {}", mail.file_path);
						}
					} else {
